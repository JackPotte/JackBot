...
eval = u'Asie du Sud-Est' # Nom à ajouter
...
def modification(PageHS):
		...
		while position != -1 and position < len(PageTemp): # On déplace les données d'une page provisoire dans une page finale jusqu'à disparition de la première
			...
			if position == -1 and projet == "True": break 	# S'il n'y a plus d'évaluation à traiter
				...
				if projet == "False":
					...
					if PageTemp.find(u'importance=') != -1 and PageTemp.find(u'importance=') < PageTemp.find("}}"):
					...
					if PageTemp3.find(u'|') != -1 and PageTemp3.find(u'|') < PageTemp3.find(u'}'):
						if PageTemp[PageTemp.find(u'|avancement='):PageTemp.find(u'|avancement=')+PageTemp3.find(u'|')+1] == u'|avancement=?':
							...
							if PageTemp4.find(u'|avancement=') != -1:
								...
								if PageTemp5.find(u'|') != -1 and PageTemp5.find(u'|') < PageTemp5.find(u'}'):
									avancement = PageTemp4[PageTemp4.find(u'|avancement='):PageTemp4.find(u'|avancement=')+PageTemp5.find(u'|')+1]
								elif PageTemp5.find(u'\n') != -1 and PageTemp5.find(u'\n') < PageTemp5.find(u'}'):
									avancement = PageTemp4[PageTemp4.find(u'|avancement='):PageTemp4.find(u'|avancement=')+PageTemp5.find(u'\n')+1]
								else:
									avancement = PageTemp4[PageTemp4.find(u'|avancement='):PageTemp4.find(u'|avancement=')+PageTemp5.find(u'}')+1]
						else:
							avancement = PageTemp[PageTemp.find(u'|avancement='):PageTemp.find(u'|avancement=')+PageTemp3.find(u'|')+1]						
						...
						if PageTemp[PageTemp.find(u'|avancement='):PageTemp.find(u'|avancement=')+PageTemp3.find(u'\n')+1] == u'|avancement=?':
							...
							avancement = PageTemp[PageTemp.find(u'|avancement='):PageTemp.find(u'|avancement=')+PageTemp3.find(u'\n')+1]
					else:
						if PageTemp[PageTemp.find(u'|avancement='):PageTemp.find(u'|avancement=')+PageTemp3.find(u'}')+1] == u'|avancement=?':
							PageTemp4 = PageTemp3[PageTemp3.find(u'|avancement=')+1:len(PageTemp3)]
							if PageTemp4.find(u'|avancement=') != -1:
	...
	if not avancement:
		PageEnd = PageEnd[0:PageEnd.find(u'{{Wikiprojet')+PageTemp.find('}}')] + u'\n|avancement=?' + PageEnd[PageEnd.find(u'{{Wikiprojet')+PageTemp.find('}}'):len(PageEnd)]
	else:
		PageEnd = PageEnd[0:PageEnd.find(u'{{Wikiprojet')+PageTemp.find('}}')] + avancement + PageEnd[PageEnd.find(u'{{Wikiprojet')+PageTemp.find('}}'):len(PageEnd)]
	# Gestion des 1% d'imprévus par le modèle
	PageTemp = PageEnd[PageEnd.find(u'{{Wikiprojet'):len(PageEnd)]
	if PageEnd[PageEnd.find(u'{{Wikiprojet')+PageTemp.find('}}')+2:PageEnd.find(u'{{Wikiprojet')+PageTemp.find('}}')+3] != u'\n':
		PageEnd = PageEnd[0:PageEnd.find(u'{{Wikiprojet')+PageTemp.find('}}')+2] + u'\n' + PageEnd[PageEnd.find(u'{{Wikiprojet')+PageTemp.find('}}')+2:len(PageEnd)]
	if PageEnd[PageEnd.find(avancement)-1:PageEnd.find(avancement)] != u'\n' and PageEnd[PageEnd.find(avancement)-1:PageEnd.find(avancement)] != u'\r':
		PageEnd = PageEnd[0:PageEnd.find(avancement)] + u'\n' + PageEnd[PageEnd.find(avancement):len(PageEnd)]
	...
	if PageTemp.find("gare|") != -1 and PageTemp.find("gare|") < PageTemp.find("}}"):
		PageEnd = PageEnd[0:PageEnd.find(u'{{Wikiprojet')+PageTemp.find("gare|")] + PageEnd[PageEnd.find(u'{{Wikiprojet')+PageTemp.find("gare|")+1:len(PageEnd)]
	if PageTemp.find("||") != -1 and PageTemp.find("||") < PageTemp.find("}}"):
...
TraitementCategory = crawlerCat(u'Catégorie:Projet:Asie du Sud-Est/Articles liés')
raw_input("Jackpot")

'''
cd "C:\Program Files\Apache Software Foundation\Apache2.2\htdocs\Personnel\mybot\pywikipedia\"
python fr.w.evaluation2.py
