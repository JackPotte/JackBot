#!/usr/bin/env python
# coding: utf-8
# Ce script gère les annexes de conjugaison
...
summary = u'Ajout des interwikis et/ou [[Modèle:Onglets conjugaison]]'
...
def modification(PageHS):
	...
	if page.exists():
		if page.namespace() !=100 and PageHS != u'Utilisateur:JackBot/test':
			return
		else:
			try:
				PageBegin = page.get()
	...
	# Italien
	if PageHS.find(u'italien') != -1:
		# Gestion des pronominaux
		if PageTemp.find(u'{{Onglets conjugaison') == -1:
			if PageHS[len(PageHS)-2:len(PageHS)] == u'si':
				pagePronom = Page(site,PageHS[0:len(PageHS)-2])
				if pagePronom.exists():
					try:
						PageTemp2 = pagePronom.get()
					except wikipedia.NoPage: return
					except wikipedia.IsRedirectPage: return
					except wikipedia.LockedPage: return
					if PageTemp2.find(u'{{Onglets conjugaison') == -1:
						PageEnd = u'{{Onglets conjugaison\n|onglet1=Forme active\n|contenu1=' + PageTemp2 + u'\n|onglet2=Forme pronominale\n|contenu2=' + PageTemp + u'\n|sél=2\n}}'
			else:
				pagePronom = Page(site,PageHS[0:len(PageHS)-1] + u'si')
				if pagePronom.exists():
					try:
						PageTemp2 = pagePronom.get()
					except wikipedia.NoPage: return
					...
					if PageTemp2.find(u'{{Onglets conjugaison') == -1:
						PageEnd = u'{{Onglets conjugaison\n|onglet1=Forme active\n|contenu1=' + PageTemp + u'\n|onglet2=Forme pronominale\n|contenu2=' + PageTemp2 + u'\n|sél=1\n}}'
		
		
		if PageEnd == u'': PageEnd = PageTemp
		# Gestion des interwikis
		site2 = getSite(u'it',family)
		PageHS2 = u'Appendice:Coniugazioni/Italiano/' + PageHS[PageHS.rfind(u'/')+1:len(PageHS)]
		page2 = Page(site2,PageHS2)
		if page2.exists() and PageEnd.find(u'[[it:') == -1:
			PageEnd = PageEnd + u'\n[[it:' + PageHS2 + u']]'
	
	if PageEnd != PageBegin:
		sauvegarde(page,PageEnd)
		if PageTemp2 != u'':
			if PageEnd.find(u'|sél=1') != -1:
				sauvegarde(pagePronom,PageEnd[0:PageEnd.find(u'|sél=1')+len(u'|sél=')] + u'2' + PageEnd[PageEnd.find(u'|sél=1')+len(u'|sél=1'):len(PageEnd)])
			elif PageEnd.find(u'|sél=2') != -1:
				sauvegarde(pagePronom,PageEnd[0:PageEnd.find(u'|sél=2')+len(u'|sél=')] + u'1' + PageEnd[PageEnd.find(u'|sél=2')+len(u'|sél=2'):len(PageEnd)])
...
# Lancement
TraitementCategorie = crawlerCat(u'Catégorie:Conjugaison en italien',False,u'')
'''
