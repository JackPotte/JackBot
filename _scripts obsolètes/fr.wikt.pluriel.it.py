#!/usr/bin/env python
# coding: utf-8
# Ce script crée les pluriels en italien
...
template = u'it-accord-ae'
summary = u'Création à partir du singulier'
...
def modification(PageHS):
	...
	pluriel = ''
	if PageSing.find(template) == -1:
		if PageSing.find(u'({{p}} : \'\'\'[[') != -1 and PageSing.find(u']]\'\'\')') != -1:
			PageTemp = PageSing[PageSing.find(u'({{p}} : \'\'\'[['):len(PageSing)]
			pluriel = PageTemp[len(u'({{p}} : \'\'\'[['):PageTemp.find(u']]')]
			if pluriel == (PageHS[0:len(PageHS)-1] + template[len(template)-1:len(template)]):
				PageTemp2 = PageSing[0:PageSing.find(u'({{p}} : \'\'\'[[')]
				if PageTemp2.rfind(u'{{=') == PageTemp2.rfind(u'{{=it='): 
					PageSing = PageSing[0:PageTemp2.rfind(u'}}\n')+3] + u'{{' + template + u'|' + PageHS[0:len(PageHS)-1] + u'}}\n' + PageSing[PageTemp2.rfind(u'}}\n')+3:PageSing.find(u'({{p}} : \'\'\'[[')] + PageSing[PageSing.find(u']]\'\'\')')+len(u']]\'\'\')'):len(PageSing)]
				else:
					return
			else:
				return
	...
	if PageTemp2.find(u'{{mf}}') != -1 and PageTemp2.find(u'{{mf}}') < PageTemp2.find(u'\n'):
		...
		if PageSing.find(u'|mf=') == -1:
			...
			result = debug(PageSing)
			if result == 'n' or result == 'no' or result == 'non': return
			page.put(PageSing, u'|mf=oui')
		elif PageSing != page.get():
			result = debug(PageSing)
			if result == 'n' or result == 'no' or result == 'non': return
			page.put(PageSing, template)
	elif PageSing != page.get():
		result = debug(PageSing)
		if result == 'n' or result == 'no' or result == 'non': return
		page.put(PageSing, template)
	...
	if PageTemp.find(u'|pp=') != -1 and PageTemp.find(u'|pp=') < PageTemp.find(u'}}'):
		...
		if PageTemp.find(u'|') == -1 or PageTemp.find(u'|') > PageTemp.find(u'}}'):
			raw_input(u'Erreur : ' + PageHS)
		else:
			PageTemp2 = PageTemp[PageTemp.find(u'|')+1:PageTemp.find(u'}}')] # On saute le radical
			if PageTemp2.find(u'|') != -1:
				pron = PageTemp2[PageTemp2.find(u'|')+1:len(PageTemp2)]
				while pron.find(u'=') != -1:
	...
	if pron != '':
		...
		pron2 = pron + template[len(template)-1:len(template)]
	else:
		pron2 = u''
	...
	if pluriel == u'': pluriel = PageHS[0:len(PageHS)-1] + template[len(template)-1:len(template)]
	page2 = Page(site,pluriel)
	if page2.exists(): 
		return
	else:
		PageEnd = u'== {{=it=}} ==\n{{-flex-' + nature + u'-|it}}\n{{' + template + u'|' + PageHS[0:len(PageHS)-1] + u'|' + pron + u'}}\n\'\'\'' + pluriel + u'\'\'\' {{pron|' + pron2 + '|it}}' + genre + u'\n# \'\'Pluriel de\'\' [[' + PageHS + u'#it|' + PageHS + u']].\n'
		while PageEnd.find(u'{{pron|it}}') != -1:
			PageEnd = PageEnd[0:PageEnd.find(u'{{pron|it}}')+7] + u'|' + PageEnd[PageEnd.find(u'{{pron|it}}')+7:len(PageEnd)]
		# Autres paragraphes
		if PageTemp.find(template) != -1:
			...
			PageEnd = PageEnd + u'\n{{-flex-' + nature2 + u'-|it}}\n{{' + template + u'|' + PageHS[0:len(PageHS)-1] + u'|' + pron + u'}}\n\'\'\'' + pluriel + u'\'\'\' {{pron|' + pron2 + '|it}}' + genre2 + u'\n# \'\'Pluriel de\'\' [[' + PageHS + u'#it|' + PageHS + u']].\n'
			while PageEnd.find(u'{{pron|it}}') != -1:
				PageEnd = PageEnd[0:PageEnd.find(u'{{pron|it}}')+7] + u'|' + PageEnd[PageEnd.find(u'{{pron|it}}')+7:len(PageEnd)]		
		# cas des locutions
		if pluriel.find(u' ') != -1: PageEnd = PageEnd[0:PageEnd.find(template)+len(template)] + u'|p=' + pluriel + PageEnd[PageEnd.find(template)+len(template):len(PageEnd)]
		# Clés de tri
	...
	if PageTemp.find(u'|p2=') != -1 and PageTemp.find(u'|p2=') < PageTemp.find(u'}}'):
		...
		if page2.exists(): 
			...
			PageEnd = u'== {{=it=}} ==\n{{-flex-' + nature + u'-|it}}\n{{' + template + u'|' + PageHS[0:len(PageHS)-1] + u'|' + pron + u'}}\n\'\'\'' + pluriel2 + u'\'\'\' {{pron|' + pron2 + '|it}}' + genre + u'\n# \'\'Pluriel de\'\' [[' + PageHS + u'#it|' + PageHS + u']].\n'
			while PageEnd.find(u'{{pron|it}}') != -1:
				PageEnd = PageEnd[0:PageEnd.find(u'{{pron|it}}')+7] + u'|' + PageEnd[PageEnd.find(u'{{pron|it}}')+7:len(PageEnd)]
		# Clés de tri
		if PageEnd.find(u'{{clé de tri') == -1:
...
def debug(PageEnd):
	...
	#return raw_input("Sauvegarder ? (o/n)")
	return u'ok'
...
def sauvegarde(PageCourante, Contenu):
	...
	try:
		result = "ok"
		#print(Contenu.encode(config.console_encoding, 'replace')[0:3000])
