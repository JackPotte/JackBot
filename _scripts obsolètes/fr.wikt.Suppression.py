#!/usr/bin/env python
# coding: utf-8
# Ce script liste remplace des modèles dans leurs pages liées, avant de les supprimer
...
def modification(PageHS):
	...
	if page.exists():
		if page.namespace() != 10 and page.title() != u'Utilisateur:JackBot/test': 
			return
		else:
			if PageHS.find(u'=') != -1: NouveauModele = u'langue|' + PageHS[1:len(PageHS)-1] + u'}}'
			...
			for PageCourante in pagegenerators.PreloadingGenerator(gen,100):
				...
				while PageTemp.find(PageHS) != -1:
					if NouveauModele == u'':
						PageTemp = PageTemp[0:PageTemp.find(PageHS)] + NouveauModele + PageTemp[PageTemp.find(PageHS)+len(PageHS):len(PageTemp)]
					else:
						PageTemp = PageTemp[0:PageTemp.find(PageHS)] + PageTemp[PageTemp.find(PageHS)+len(PageHS):len(PageTemp)]
				if PageTemp != PageBegin: sauvegarde(PageCourante.title(),PageTemp)
			#page.delete(u'Suppression après gestion des pages liées', not self.always, throttle = True)	
...
# Lancement
TraitementCategory = crawlerCat(u'Plurale tantum')
TraitementCategory = crawlerCat(u'Wiktionnaristes')
'''
...
python protect.py -lang:fr -family:wiktionary -file:"articles_list.txt" -edit:sysop -move:sysop -summary:"Protection des modèles > 10 000"
python delete.py -lang:fr -family:wikibooks -file:"articles_list.txt" -summary:
python movepages.py -lang:fr -family:wiktionary -pairs:"articles_listed.txt" -noredirect
python interwiki.py -lang:fr -family:wikibooks -page:"Utilisateur:JackBot"
python imagecopy.py -lang:fr -family:wikibooks -cat:
python revertbot.py -lang:fr -family:wiktionary
