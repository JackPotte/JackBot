...
def create_item(lang, title, token=None,check=True,labels=False):
    ...
    p2 = {'action':'wbeditentity',
          'data':export,
          'summary':'Importing from [[:'+fam+':{0}:{1}]]'.format(lang, title),
          'bot':1
    }
    if token:
        p2['token'] = token
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        category=pywikibot.Category(Site, sys.argv[1])
    else:
        category=pywikibot.Category(Site, u'Maintenance Wikinews')
    for subcat in category.subcategories(recurse=True):
        print subcat
        x=create_item(language,subcat.title(),labels=True)

# python fr.n.wikidata_create.py 