...
def hyperlynx(PageTemp, debugLevel = 0):
	...
	if debugLevel > 0: print u' Langue de la page distante : ' + languePage
	try:
...
def TestURL(url, debugLevel = 0):
	...
	for blacklisted in brokenDomain:
...
def TestPage(htmlSource, url, debugLevel = 0):
	...
	try:
		...
		if htmlSource is None:
			...
			return False
		elif htmlSource == u'ko' or htmlSource == u'': #and url[len(url)-4:len(url)] != u'.pdf':
