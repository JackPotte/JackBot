...
siteDest = pywikibot.Site(site_language, site_family)
...
def treat_page_by_name(page_name):
    ...
    if language_code == 'LL':
        ...
        if s:
            language_code = getlanguage_codeISO693_1FromISO693_3(s.group(1))
        else:
    ...
    try:
        ...
        if debug_level > 1: print(' Mot potentiel : ') + word
        ...
        if word != word1:
            ...
            try:
                ...
                if debug_level > 0: print(' Page not found 1')
                return
            except pywikibot.exceptions.IsRedirectPage:
                current_page_content = page1.get(get_redirect=True)
        else:
            if debug_level > 0: print(' Page not found 2')
            return
    except pywikibot.exceptions.IsRedirectPage:
        current_page_content = page1.get(get_redirect=True)
    ...
    if debug_level > 1: print(' Mot du Wiktionnaire : ') + word
...
def main(*args):
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == '-test' or sys.argv[1] == '-t':
            ...
            p.pages_by_cat('Canadian English pronunciation', afterPage = afterPage, recursive = True, namespaces = [6])
            p.pages_by_cat('Australian English pronunciation', afterPage = afterPage, recursive = True, namespaces = [6])
            p.pages_by_cat('British English pronunciation', afterPage = afterPage, recursive = True, namespaces = [6])
            p.pages_by_cat('U.S. English pronunciation‎', afterPage = afterPage, recursive = True, namespaces = [6])
            #Bug: too long? p.pagesByCat('Lingua Libre pronunciation-fr', afterPage = afterPage, recursive = True, namespaces = [6])
