...
list_homophons = False
do_fix_translations = False
...
def treat_page_by_name(page_name):
    ...
    if treat_templates and page.namespace() == 10:
        ...
        for template in templates_to_treat:
            if '{{{clé|' not in page_content and page_content[:len(
        ...
        if re.search(regex, page_content):
            if debug_level > 0:
                print('Page en travaux : non traitée l 1409')
            return
        ...
        page_content, summary = addSeeBanner(page_name, page_content, summary)
        page_content, summary = formatSections(page_content, summary)
        ...
        if debug_level > 0:
            print(' Flexions formatting')
        if debug_level > 1:
            print('  fr')
        regex = r"(=== {{S\|nom\|fr)\|flexion(}} ===\n(?:{{fr[^\n]*\n)*'''" + regex_page_name + r"''' [^\n]*{{fsing}})"
        if re.search(regex, page_content):
        ...
        if page_name.find('*') == -1 and page_name[-1:] == 's':
            page_content, final_page_content, summary = treat_noun_inflexion(page_content, final_page_content, summary,
                                                                             page_name, regex_page_name)

        if debug_level > 0:
            print(' Missing translations')
        # Si la définition du mot (dit "satellite") ne renvoie pas vers un autre, les centralisant
        # TODO: # Variante,
        regex = r'(fr\|flexion|' + '|'.join(definitionSentences) + '|'.join(
        ...
        French, language_start, language_end = getLanguageSection(page_content, 'fr')
        ...
        add_language_code = False  # Certaines sections interdisent les modèles de domaine catégorisant
        if debug_level > 1:
            print('  add_language_code = ') + str(add_language_code)
        has_translation_section = False
        go_backward = False  # Certains modèles nécessitent d'être déplacés puis retraités
        language_code = None
        if debug_level > 1:
            ...
            if current_template in templates:
                ...
                if not language_code and (p < limit1 or p >= limit6) and current_template != 'ébauche':
                    ...
                    final_page_content = '{{formater|Section de langue manquante, avant le modèle ' \
                                         ...
                                         + '-ème caractère)}}\n' + final_page_content + page_content
                    summary = 'Page à formater manuellement'
                    ...
                    if language_code == 'conv':
                        ...
                        if page_content.find('{{S|anagr') == -1 and page_name.find(' ') == -1 and len(
                            ...
                            page_content = add_anagrams(page_content, summary, page_name, language_code)
                    final_page_content, page_content = nextTemplate(final_page_content, page_content)
                ...
                # Sections
                    ...
                    if section not in sections:
                        if debug_level > 0:
                            pywikibot.output(u"\03{yellow} S\03{default}: with unknown section " + section)
                        final_page_content, page_content = nextTemplate(final_page_content, page_content)
                        break
                    if debug_level > 0:
                        pywikibot.output(u"\03{yellow} S\03{default}: with known section " + section)
                    ...
                    if sections.index(section) < limit1:
                        ...
                        if page_content.find('|clé=') == -1 or page_content.find('|clé=') > page_content.find('}}'):
                            ...
                            if temp_page_name != page_name:
                                if debug_level > 0:
                                    print(' "|clé="')
                                temp_page_name = default_sort(temp_page_name)
                    ...
                    if term == '':
                        ...
                        if debug_level > 0:
                            print("  1 = " + term)
                        template_page = get_content_from_page_name('Template:' + term, allowedNamespaces=['Template:'])
                        if template_page.find('Catégorie:Modèles de domaine') == -1 and \
                            ...
                            if debug_level > 0:
                                print('  2 = ') + term
                            template_page = get_content_from_page_name('Template:' + term,
                                                                       allowedNamespaces=['Template:'])
                ...
                # Templates with language code at second
                    ...
                    if language_code == 'conv':
                        ...
                        if debug_level > 0:
                            pywikibot.output(u"  Template with language code at second: \03{green}" + current_template
                                             + u"\03{default}")
                ...
                # Templates with two parameters
                    ...
                    if final_page_content.rfind('{{') != -1:
                        ...
                        if add_language_code and (
                                (final_page_content2.rfind('{{') != final_page_content2.rfind('{{pron|') and \
                                 final_page_content2.rfind('{{') != final_page_content2.rfind(
                                            '{{US|') and final_page_content2.rfind('{{') != final_page_content2.rfind(
                                            '{{UK|')) \
                                or final_page_content.rfind('{{pron|') < final_page_content.rfind(
                            '\n') or final_page_content2.rfind('{{pron|') == -1) \
                                and ((page_content.find('{{') != page_content.find('{{pron|') or page_content.find(
                            '{{pron|') > page_content.find('\n')) \
                ...
                if fix_old_templates:
                    ...
                    template_page = get_content_from_page_name('Template:' + current_template,
                                                               allowedNamespaces=['Template:'])
                    ...
                    if debug_level > 0:
                        pywikibot.output(u"\03{yellow} " + current_template + "\03{default}: unknown template")
                    final_page_content, page_content = nextTemplate(final_page_content, page_content)
    ...
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == str('-test'):
            ...
            if test_page is not None:
                page_content = get_content_from_page_name(test_page)
            ...
            if len(sys.argv) > 4:
                ...
                number = 1000
            p.pages_by_user('User:' + targeted_user, numberOfPagesToTreat=number, namespaces=[0])
            ...
            if len(sys.argv) > 2:
                ...
                sys.argv[1] == str('-m'):
            p.pages_by_link('Template:contr', namespaces=[0])
            p.pages_by_link('Template:deet', namespaces=[0])
            ...
            if len(sys.argv) > 2:
                if sys.argv[2] == str('listFalseTranslations'):
                    ...
                    p.pages_by_cat('Appels de modèles incorrects:abréviation', afterPage=after_page, recursive=False,
                                   namespaces=[14])
                else:
