...
# Déclaration
mynick = "JackBot"
siteDest = getSite(u'it',u'wiktionary')
site = getSite(u'',u'commons')
summary = u'Aggiunta di suono da [[commons:Category:Pronunciation]]'
debogage = False
ns = 0
...
def modification(PageHS):
	...
	if PageTemp.find(u'{{-pron-}}') != -1 and ((PageTemp.find(u'{{-pron-}}') < PageTemp.find(u'{{langue') or PageTemp.find(u'{{langue') == -1)):
		PageEnd = PageEnd + PageTemp[:PageTemp.find(u'{{-pron-}}')+len(u'{{-pron-}}')]
		PageTemp = u'\n* {{pron-rég|lang=' + codelangue + u'|' + region + u'|audio=' + PageHS[5:] + u'}}\n' + PageTemp[PageTemp.find(u'{{-pron-}}')+len(u'{{-pron-}}'):]
		...
		PageEnd = PageEnd[:PageEnd.rfind(u'\n\n')] + u'\n\n{{-pron-}}\n* {{pron-rég|lang=' + codelangue + u'|' + region + u'|audio=' + PageHS[5:] + u'}}' + PageEnd[PageEnd.rfind(u'\n\n'):]
