...
# Déclaration
mynick = "JackBot"
site = getSite("fr","wikipedia")
summary = u'[[Wikipédia:Bot/Requêtes/2012/08#divisions administratives du Botswana]]'
sep = u','
Contenu = range(1, 7+2)

# Modification du wiki
def modification(PageHS):
	#PageHS = csonome,nome,sous-district,popolazione,localita,latitudine,longitudine
	PageBegin = u''
	for c in range(1,7+1):
		Contenu[c] = PageHS[0:PageHS.find(sep)]
		PageHS = PageHS[PageHS.find(sep)+1:len(PageHS)]
		#print (Contenu[c] + u'\n')
	page = Page(site,Contenu[2])
	PageEnd = u''
	if page.exists():
		try:
			PageTemp = page.get()
			...
			return
		if PageTemp.find(u'Infobox Ville') == -1:
			#page = Page(site,Contenu[2] + u' (ville du Botswana)')
			#if page.exists(): return
			print Contenu[2]
			return
		else:
			return
	site2 = getSite("it","wikipedia")
	page2 = Page(site2,Contenu[2])
	if page2.exists():
		try:
			PageTemp2 = page2.get()
		except wikipedia.NoPage:
			...
			print "Locked/protected page"
			return
		if PageTemp2.find(u'[[fr:') != -1: return
	
	PageTemp = u'{{Infobox Ville2\n| nom=' + Contenu[2] + u'\n| carte=\n| pays=Botswana\n| nom de division=Sous-district\n| division=[[' + Contenu[3] + u']]\n| population=' + Contenu[4] + u'\n| année_pop=2011\n| latitude = ' + Contenu[6].replace(u'"',u'') + u'\n| longitude = ' + Contenu[7].replace(u'"',u'') + u'\n}}\n\'\'\'' + Contenu[2] + u'\'\'\' est une ville du [[Botswana]].\n\n== Notes et références ==\n{{Références}}\n*{{en}} http://www.cso.gov.bw/media/2011%20Census%20_Alphabetical%20Index%20_Population%20of%20Localities.pdf\n{{Share Your Knowledge}}\n\n{{Portail|Botswana}}\n\n[[Catégorie:Ville du Botswana]]\n\n[[it:' + Contenu[2] + u']]'
...
# Lancement
TraitementFile = crawlerFile('_Villaggi.csv')
'''
